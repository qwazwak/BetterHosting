...
internal sealed partial class AsyncManualResetEvent<T>
{
    ...
    public void Reset()
    {
        TaskCompletionSource<T> prevTCS;
        lock (syncObject)
        {
    ...
    public void SetCancelled(CancellationToken reason) => SetCore(LockState.Cancelled, tcs => tcs.SetCanceled(reason));
    public void SetFailed(Exception exception) => SetCore(LockState.Failed, tcs => tcs.SetException(exception));
    private void SetCore(LockState newState, Action<TaskCompletionSource<T>> transition)
    {
