...
namespace QLib.AsyncEventBlock;

public sealed partial class OneTimeAsyncEvent<T>
{
...
#if AsyncManualResetEventWithValue
    public void SetOpen(T value) =>  SetCore(LockState.SetOpen, transition: tcs => tcs.TrySetResult(value), value: value);
#else
    public void SetOpen() => SetCore(LockState.SetOpen, transition: tcs => tcs.TrySetResult());
#endif
...
#if AsyncManualResetEventWithValue
    private void SetCore(LockState newState, Func<TaskCompletionSource<T>, bool> transition, T? value = default!)
#else
